...
import { Trash2, Clock, DollarSign, Check, Smartphone, RefreshCw, X, Gift, Wallet } from 'lucide-react';
import Header from './header.jsx';
import Swal from 'sweetalert2';
import { useWallet } from './Walletcontext';

// ─────────────────────────────────────────────────────────────────────────────
// Accept Bid Popup — shows right after user clicks "Accept Bid"
// ─────────────────────────────────────────────────────────────────────────────
          ...
          >
            ...
            <div className="text-left flex-grow">
              ...
              <p className="text-gray-600 text-xs font-semibold mt-0.5">
                Earn <span className="text-orange-600 font-black">${bonus} extra</span> — Total ${withBonus}
              </p>
            </div>
          </button>
...
const WithdrawModal = ({ isOpen, onClose, amount, orderId, onSuccess }) => {
  const { withdrawCash, isProcessing } = useWallet();
      ...
      >
        <div className="bg-gradient-to-r from-green-600 to-green-700 p-5 relative">
          <button
            ...
            className="absolute top-3 right-3 text-white/70 hover:text-white disabled:opacity-40"
          >
            <X size={20} />
          </button>
          <div className="flex items-center gap-3">
        ...
        <div className="p-5">
          {submitted ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-gray-900 font-black text-lg mb-2 uppercase">Submitted!</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button
                  type="submit"
                  disabled={isProcessing}
                  className="flex-1 px-6 py-3 bg-green-600 text-white rounded-xl font-black text-sm uppercase hover:bg-green-700 transition-all disabled:opacity-50"
                >
        ...
        <div className="p-5">
          {submitted ? (
            <div className="text-center py-6">
              ...
              <p className="text-gray-600 text-sm font-semibold">
                Coupons sent to your registered email.
              </p>
            </div>
          ) : (
...
const MobileCart = () => {
  ...
  const { addToWallet, walletBalance } = useWallet();
      ...
      try {
        ...
        if (response.ok) {
          ...
          localStorage.setItem(
            'myGuestOrders',
            JSON.stringify(guestOrders.filter(oid => oid !== id))
          );
  ...
  const handleUpdateStatus = async (id, newStatus, bidPrice = 0) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (newStatus === 'accepted' && bidPrice > 0) {
          const acceptedOrders = JSON.parse(localStorage.getItem('acceptedOrders') || '[]');

          if (!acceptedOrders.includes(id)) {
            // Add to wallet with orderId for per-order tracking
            addToWallet(bidPrice, id);
            ...
            // Show custom popup (not Swal)
            setAcceptPopup({ open: true, orderId: id, bidPrice });
          } else {
            Swal.fire({
              icon: 'info',
              title: 'Already Added',
              text: 'This order has already been added to your wallet.',
  ...
  // AcceptPopup → WithdrawModal
  const handlePopupWithdraw = () => {
    const { orderId, bidPrice } = acceptPopup;
    setAcceptPopup({ open: false, orderId: null, bidPrice: 0 });
    setWithdrawModal({ open: true, orderId, amount: bidPrice });
  };

  // AcceptPopup → CouponModal
  const handlePopupCoupon = () => {
    const { orderId, bidPrice } = acceptPopup;
    setAcceptPopup({ open: false, orderId: null, bidPrice: 0 });
    setCouponModal({ open: true, orderId, amount: bidPrice });
  };

  const handleWithdrawSuccess = () => {
    setTimeout(() => setWithdrawModal({ open: false, orderId: null, amount: 0 }), 0);
  };

  const handleCouponSuccess = () => {
    setTimeout(() => setCouponModal({ open: false, orderId: null, amount: 0 }), 0);
  };
  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen pb-10 font-sans">
      ...
      {/* ── Modals ── */}
      <AcceptPopup
        isOpen={acceptPopup.open}
        onClose={() => setAcceptPopup({ open: false, orderId: null, bidPrice: 0 })}
        bidPrice={acceptPopup.bidPrice}
        orderId={acceptPopup.orderId}
        onWithdraw={handlePopupWithdraw}
        onCoupon={handlePopupCoupon}
      />

      <WithdrawModal
        isOpen={withdrawModal.open}
        onClose={() => setWithdrawModal({ open: false, orderId: null, amount: 0 })}
        amount={withdrawModal.amount}
        orderId={withdrawModal.orderId}
        onSuccess={handleWithdrawSuccess}
      />

      <CouponModal
        isOpen={couponModal.open}
        onClose={() => setCouponModal({ open: false, orderId: null, amount: 0 })}
        amount={couponModal.amount}
        orderId={couponModal.orderId}
          ...
          <div className="mb-4 bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-200 rounded-2xl p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase text-green-700 mb-0.5">
          ...
          {cartItems.length === 0 && !loading ? (
            ...
            cartItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:border-blue-200 transition-all duration-300"
                >
                  <div className="p-4 md:p-5 flex flex-col md:flex-row gap-5 items-center md:items-start">
                    ...
                    <div className="flex-grow w-full text-center md:text-left">
                      ...
                      <div className="flex flex-col md:flex-row justify-between items-center md:items-start gap-2 mb-3">
                        <div>
                          <h3 className="text-lg font-black text-gray-800 leading-none uppercase tracking-tight">
                            {item.brand} {item.name}
                          </h3>
                          <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">
                            Requested on {item.uploadDate}
                          </p>
                        </div>
                      ...
                      {hasBid && !isRejected ? (
                        <div
                          className={`rounded-xl p-3 flex flex-col md:flex-row items-center justify-between gap-4 ${
                            isAccepted ? 'bg-green-50/50' : 'bg-blue-50'
                          }`}
                        >
                          <div className="flex items-center gap-3">
                            <div
                            ...
                            <div className="text-left">
                              <p className="text-[10px] font-bold uppercase text-gray-400 leading-none mb-1">
                                Final Price Offer
                              </p>
                              <p
                                className={`text-2xl font-black ${
                                  isAccepted ? 'text-green-700' : 'text-blue-700'
                                }`}
                              >
                                ${item.bidPrice}
                              </p>
                          ...
                          {isAccepted ? (
                            ...
                            <div className="flex gap-3 w-full md:w-auto">
                              <button
                                onClick={() => handleUpdateStatus(item.id, 'accepted', item.bidPrice)}
                                disabled={isItemProcessing}
                                className="flex-1 md:px-6 py-2.5 bg-green-600 text-white text-[11px] font-black uppercase rounded-xl hover:bg-green-700 active:scale-95 transition-all flex items-center justify-center gap-2 tracking-wider disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {isItemProcessing ? (
                              ...
                              <button
                                ...
                                disabled={isItemProcessing}
                                className="flex-1 md:px-6 py-2.5 bg-white border-2 border-red-50 text-red-500 text-[11px] font-black uppercase rounded-xl hover:bg-red-50 active:scale-95 transition-all tracking-wider disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                Reject
                              </button>
                            </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-between gap-3 rounded-2xl p-4 bg-amber-50 border border-amber-100/50">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="text-xs font-bold text-amber-800 tracking-tight">
                              Awaiting Admin Verification & Price Offer
                            </p>
                          </div>
                          <button
...
const Badge = ({ status }) => {
  ...
  return (
    <span
      className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-wider ${s.bg} ${s.text} border border-white/50 shadow-sm`}
    >
      {s.label}
    </span>
  );
